...
.methods({
  ...
  uriFor (search, scope, field, format) {
    ...
    queryParams['search_scope'] = primouiScope || 'mncat_discovery';

    let primouiField;
    if (field) {
      const fields = this.fields();
      if (field in fields) {
        primouiField = fields[field];
      } else {
        warnings.push('Unrecognized field: "' + field + '"');
      }
    }
    queryParams['query'] = (primouiField || 'any') + ',contains,' + search;
...
module.exports = plugin.compose(primoui);
