...
const archivespace = stampit()
  .methods({
    ...
    uriFor (search, scope, field) {
      ...
      const queryParams = {
        'op[]': '', // ArchiveSpace requires this param, even if it has no value.
        'q[]': search
      }
      ...
      if (field && !badField) {
        if (field === 'subject') {
          queryParams.limit = this.fields()[field]
        } else {
      ...
      if (scope && !badScope) {
        segments.unshift('repositories', scope)
      }
...
export default plugin.compose(archivespace);
